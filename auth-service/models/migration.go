	...
	for _, service := range services {
		...
		ensureExternalRole(ctx, service.Key, "user_manager",
			...
			[]string{
				...
				fmt.Sprintf("auth.%s.service_roles.view", service.Key),
				fmt.Sprintf("auth.%s.service_roles.create", service.Key),
				fmt.Sprintf("auth.%s.service_roles.edit", service.Key),
				fmt.Sprintf("auth.%s.service_roles.delete", service.Key),
